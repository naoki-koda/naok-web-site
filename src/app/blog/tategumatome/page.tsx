...
const selectionCriteria = [
  ...
  },
  {
    title: "設計～施工の伴走力",
    detail:
      "ヒアリングから図面・3D提案まで内製しているか、打ち合わせの工数を確認。",
  },
  {
    title: "施工エリアとアフター",
...
const ranking = [
  ...
  },
  {
    ...
    tagline:
      "自社工場×デザイナー直対応。天然石も扱う高難度造作を得意とする上場",
  ...
  },
  {
    ...
    name: "LUFT（ルフトインテリア）",
    tagline:
      "オーダー家具とインテリアコーディネートを一体で頼める仕立てる家具店",
...
const dataSources = [
  {
    name: "Web幹事「名古屋市の優良ホームページ制作会社20社」",
    url: "https://web-kanji.com/posts/nagoya-city",
  },
      ...
      <article className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
        <section className="rounded-3xl bg-white/80 p-8 shadow-xl ring-1 ring-amber-100">
          <p className="text-sm font-semibold tracking-widest text-amber-600">
            Nagoya Built-in Furniture Guide
          </p>
          <h1
            className={`mt-4 text-3xl font-bold text-slate-900 sm:text-4xl ${kosugiMaru.className}`}
          >
            名古屋のおすすめ造作家具店まとめ
          </h1>
          <p className="mt-6 text-lg leading-relaxed text-slate-700">
            Web幹事のまとめ記事形式を参考に、{ranking.length}社を横並びで調査。
            第一位には
        ...
        <section className="space-y-10">
          {ranking.map((shop, index) => (
            <React.Fragment key={shop.rank}>
              ...
              {index === 1 && <RakutenAffiliateAd ad={rakutenAds[4]} />}
            </React.Fragment>
          ))}
        </section>
        ...
        <section className="mt-16 rounded-3xl bg-slate-900 p-8 text-slate-100">
          ...
          <p className="mt-6 text-sm leading-relaxed text-slate-300">
            ...
            、<span className="text-white">③ 納期と予算の許容幅</span>{" "}
            をセットで伝えると打ち合わせがスムーズです。
          </p>
        </section>
